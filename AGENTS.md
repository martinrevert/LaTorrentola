...
- Translucent Status Bar & Top Header (Haze 2.0): To achieve a translucent glass blur status bar and top app bar, initialize `val hazeState = rememberHazeState()`. Apply `Modifier.hazeSource(state = hazeState)` to the full-screen scrollable content container, set `TopAppBarDefaults.topAppBarColors(containerColor = Color.Transparent)`, and apply `Modifier.hazeGlass(input = HazeInput.Sources(hazeState))` to the top header / `TopAppBar`.
